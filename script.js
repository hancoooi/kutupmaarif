...
https://en.wikipedia.org/wiki/Stephen_Borough`
  ...
  16: {
    baslik: "ROALD ENGELBREGT GRAVNING AMUNDSEN DOĞUMU (1872)",
  ...
  31: {
    baslik: "TASE-V / Valdivia (Carl Chun) Derin Deniz Seferi",
...
function kaynaklariLinkeCevir(kaynakMetni) {
  if (!kaynakMetni || !kaynakMetni.trim()) return 'Kaynak yok';
  return kaynakMetni
    .split('\n')
    .map((url) => url.trim())
    .filter(Boolean)
...
function loadMonth() {
  ...
  for (let i = 1; i <= 31; i += 1) {
    ...
    row.innerHTML = `
      ...
      <div class="card">${month.charAt(0).toUpperCase() + month.slice(1)} - ${temmuz[i]?.baslik || 'Henüz içerik yok'}</div>
    `;
...
function showDay(month, day, el) {
  ...
  if (!data || (!data.baslik && !data.metin && !data.kaynak)) {
  ...
  const metinHtml = (data.metin || '').replace(/\n/g, '<br>');
  const kaynakHtml = kaynaklariLinkeCevir(data.kaynak || '');

  box.innerHTML = `
    <h2>${data.baslik || 'Başlıksız'}</h2>
    <p>${day} ${month.charAt(0).toUpperCase() + month.slice(1)} tarihinde gerçekleşen olay.</p>
    <p>${metinHtml}</p>
    <p><strong>Kaynak:</strong><br>${kaynakHtml}</p>
  `;
}
